...
```bash
yorkshire-golf$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
yorkshire-golf$ sam local start-api
yorkshire-golf$ curl http://localhost:3000/
```
...
```bash
yorkshire-golf$ sam logs -n HelloWorldFunction --stack-name yorkshire-golf --tail
```
...
```bash
yorkshire-golf$ cd HelloWorldFunction
...
```bash
sam delete --stack-name yorkshire-golf
```
